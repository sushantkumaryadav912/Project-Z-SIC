    ...
    return (
        <ScrollView className="flex-1 bg-[#f6f7f8] dark:bg-slate-950">
            <View className="px-5 py-6">
                ...
                >
                    <Text className="text-2xl font-bold text-gray-900 dark:text-slate-100">{normalizedTiffin.name || 'Unnamed Tiffin'}</Text>
                    ...
                    {normalizedTiffin.description && (
                        <Text className="text-base text-gray-700 dark:text-slate-200 mt-4 leading-6">{normalizedTiffin.description}</Text>
                    )}
                ...
                >
                    <Text className="text-base font-semibold text-gray-900 dark:text-slate-100 mb-2">Meal plans</Text>
                    {plans.length > 0 ? (
                        plans.map((plan) => (
                            <Text key={plan} className="text-sm text-gray-600 dark:text-slate-300 mb-1">• {plan}</Text>
                        ))
                    ) : (
                        <Text className="text-sm text-gray-600 dark:text-slate-300">Plans to be announced.</Text>
                    )}
                </View>
                ...
                {days.length > 0 && (
                    <View
                        ...
                        style={[isDark ? null : { shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 10, elevation: 1 }]}
                    >
                        <Text className="text-base font-semibold text-gray-900 dark:text-slate-100 mb-2">Meal schedule</Text>
                        <Text className="text-sm text-gray-600 dark:text-slate-300">{days.join(', ')}</Text>
                    </View>
                )}
                ...
                {coverage.length > 0 && (
                    <View
                        ...
                        style={[isDark ? null : { shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 10, elevation: 1 }]}
                    >
                        <Text className="text-base font-semibold text-gray-900 dark:text-slate-100 mb-2">Coverage areas</Text>
                        <Text className="text-sm text-gray-600 dark:text-slate-300">{coverage.join(', ')}</Text>
                    </View>
                )}
                ...
                {(normalizedTiffin.contact || normalizedTiffin.contactPhone || normalizedTiffin.contactEmail || normalizedTiffin.location?.address) && (
                    <View
                        ...
                        style={[isDark ? null : { shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 10, elevation: 1 }]}
                    >
                        <Text className="text-base font-semibold text-gray-900 dark:text-slate-100 mb-2">Contact</Text>
                        {normalizedTiffin.contact && <Text className="text-sm text-gray-600 dark:text-slate-300 mb-1">Contact: {normalizedTiffin.contact}</Text>}
                        {normalizedTiffin.contactPhone && <Text className="text-sm text-gray-600 dark:text-slate-300 mb-1">Phone: {normalizedTiffin.contactPhone}</Text>}
                        {normalizedTiffin.contactEmail && <Text className="text-sm text-gray-600 dark:text-slate-300 mb-1">Email: {normalizedTiffin.contactEmail}</Text>}
                        {normalizedTiffin.location?.address && <Text className="text-sm text-gray-600 dark:text-slate-300">Address: {normalizedTiffin.location.address}</Text>}
                    </View>
                )}
